...
renderer.code = function (token) {
    ...
    if (language === 'data-chart' || language === 'data-table' || language === 'data-metric' || language === 'data-comparison') {
...
export function renderMarkdown(text) {
    ...
    const cleanHtml = DOMPurify.sanitize(rawHtml, {
        ALLOWED_TAGS: ['p', 'br', 'strong', 'em', 'code', 'pre', 'ul', 'ol', 'li', 'a', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'blockquote', 'table', 'thead', 'tbody', 'tr', 'th', 'td', 'img', 'span', 'div', 'hr', 'sub', 'sup', 'del', 'input'],
        ALLOWED_ATTR: ['href', 'class', 'src', 'alt', 'title', 'target', 'rel', 'checked', 'type', 'disabled'],
    });
    return cleanHtml;
}
export function highlightCode(el) { Prism.highlightAllUnder(el); }
export function extractUrls(text) { return [...new Set(text.match(/https?:\/\/[^\s\)\]]+/g) || [])]; }
// ═══ Entity Detection ═══
const ENTITY_MAP = {
    'HDB': 'company', 'ACB': 'company', 'VCB': 'company', 'TCB': 'company', 'MBB': 'company',
    'BID': 'company', 'CTG': 'company', 'VPB': 'company', 'STB': 'company', 'EIB': 'company',
    'VNINDEX': 'index', 'HNXINDEX': 'index', 'UPCOMINDEX': 'index',
    'NIM': 'metric', 'ROE': 'metric', 'ROA': 'metric', 'NPL': 'metric', 'CAR': 'metric',
    'LDR': 'metric', 'CIR': 'metric', 'PPOP': 'metric',
};
...
// ═══ Data Card Parsing ═══
export function parseDataCards(html) {
    const cards = [];
    ...
    placeholders.forEach(ph => {
        ...
        try {
            ...
            switch (type) {
                case 'chart':
                    cards.push({ type: 'chart', title: json.title, data: json });
                    break;
                case 'table':
                    cards.push({ type: 'table', title: json.title, data: json });
                    break;
                case 'metric':
                    cards.push({ type: 'metric', data: json });
                    break;
                case 'comparison':
                    cards.push({ type: 'comparison', title: json.title, data: json });
                    break;
            }
        }
