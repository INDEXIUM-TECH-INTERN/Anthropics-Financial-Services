...
function trapFocus(e) {
    if (e.key !== 'Tab')
        return;
    const f = settingsModal.querySelectorAll('input, textarea, button, select');
    if (!f.length)
        return;
...
function showMetricsModal(met) {
    if (!metricsModal)
        return;
    const lat = met.latency_ms ? (met.latency_ms / 1000).toFixed(2) + 's' : '—';
    $('metric-val-latency').textContent = lat;
    $('metric-val-token-in').textContent = String(met.token_in);
    $('metric-val-token-out').textContent = String(met.token_out);
    $('metric-val-ram').textContent = met.ram_mb || '—';
    $('metric-val-cpu').textContent = met.cpu_load || '—';
...
metricsModal?.addEventListener('click', (e) => { if (e.target === metricsModal)
    hideMetricsModal(); });
// ═══ BACKEND SELECTOR ═══
backendSelect?.addEventListener('change', (e) => {
    ...
    setupEventSource();
    store.setState({ currentBackend: b });
...
function setupEventSource() {
    ...
    eventSource.onmessage = (e) => {
        try {
            ...
            addLogEntry(d.payload || '', d.type || 'info');
            updatePipeline(d);
        }
        catch { /* */ }
    };
...
function updatePipeline(d) {
    ...
    const m = d.metadata || {};
    if (d.type === 'agent_selected')
        store.setPipeline({ agent: m.agent || msg, agentStatus: 'active', reason: m.reason || '', reasonStatus: 'active' });
    if (d.type === 'skill_loaded')
        store.setPipeline({ skill: m.skill || msg, skillStatus: 'active' });
    if (d.type === 'tool_executed')
        store.setPipeline({ tool: m.tool || msg, toolStatus: 'active' });
}
// ═══ CONVERSATIONS ═══
...
function renderSidebarConversations(searchQuery = '') {
    renderConversationList(conversationsList, store.getState().chats, store.getState().currentChatId, (id) => switchChat(id), async (id) => { if (confirm('Xóa?'))
        await deleteChat(id); }, searchQuery);
}
...
async function createNewChat(title = 'Cuộc trò chuyện mới') {
    try {
        ...
        const fb = { id: 'local_' + Date.now(), title };
...
function groupHistoryMessages(history) {
    ...
    for (let i = 0; i < history.length; i++) {
        ...
        if (last && last.role === 'bot') {
            ...
            const metrics = msg.token_in !== undefined && msg.token_in > 0 ? {
                token_in: msg.token_in, token_out: msg.token_out || 0,
                ram_mb: msg.ram_mb || '', latency_ms: msg.latency_ms, cpu_load: msg.cpu_load
            } : undefined;
...
async function switchChat(id) {
    ...
    if (currentChatTitle)
        currentChatTitle.textContent = chat?.title || id;
    clearChatUI();
    renderSidebarConversations();
    ...
    try {
        ...
        for (const g of grouped) {
            ...
            msgs[msgs.length - 1].style.animationDelay = `${idx * 0.06}s`;
            idx++;
            has = true;
        }
        if (has && welcomeState)
...
function clearChatUI() {
    ...
    if (logStream)
        logStream.innerHTML = '<div class="log-stream-empty">Sẵn sàng.</div>';
    if (sourcesPanel)
...
// ═══ MESSAGES ═══
function appendMessageBubble(text, sender, isStreaming = false, metrics) {
    if (!chatContent)
        return;
    if (welcomeState)
        welcomeState.style.display = 'none';
    ...
    if (sender === 'user' && !isStreaming) {
        el.addEventListener('click', function handler(evt) {
            if (evt.target.closest('.edit-message-container'))
                return;
            ...
            const body = el.querySelector('.msg-body');
            if (body)
                body.insertBefore(cont, content.nextSibling);
            ta.focus();
            ...
            sb.addEventListener('click', (e) => {
                ...
                while (sib) {
                    ...
                    if (sib.classList.contains('message') || sib.classList.contains('followup-chips'))
                        sib.remove();
                    sib = n;
                }
                el.remove();
...
async function sendMessage(textOverride = null, isRegenerate = false) {
    ...
    const st = Date.now();
    const { content, footer } = appendMessageBubble('', 'bot', true);
    ...
    try {
        ...
        let full = '';
        await api.streamChat(text, store.getState().currentChatId || undefined, att, abortController.signal, (tok) => {
            full += tok;
        ...
        if (currentBotContentEl)
            currentBotContentEl.innerHTML = renderErrorHTML(d);
        if (currentBotFooterEl)
            currentBotFooterEl.innerHTML = `<span class="msg-timer" style="color:var(--danger)">Thất bại · ${el}s</span>`;
        currentBotContentEl = null;
        currentBotFooterEl = null;
    }
    finally {
...
function diagnoseError(name, msg) {
    ...
    if (m.includes('failed to fetch') || m.includes('network')) {
        ...
        t = 'Không kết nối';
        i = '📡';
        d = 'Không thể kết nối đến máy chủ. Vui lòng thử lại sau.';
...
function scrollToBottom() {
    requestAnimationFrame(() => { if (chatViewport)
...
document.querySelectorAll('.welcome-chip[data-quick]').forEach(c => {
    c.addEventListener('click', () => {
        if (chatInput)
            chatInput.value = c.dataset.quick || '';
        sendMessage();
    });
});
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Escape') {
        if (settingsModal && !settingsModal.classList.contains('hidden'))
            hideSettings();
        else if (metricsModal && !metricsModal.classList.contains('hidden'))
            hideMetricsModal();
...
shortcutsPanel?.addEventListener('click', (e) => { if (e.target === shortcutsPanel)
    shortcutsPanel.classList.add('hidden'); });
