...
func NewAgentService(agent *entities.Agent, conversation *entities.Conversation, orchestrator interfaces.Orchestrator, provider interfaces.LLMProvider) *AgentServiceImpl {
...
func (s *AgentServiceImpl) Process(ctx context.Context, req *interfaces.ProcessRequest) (*interfaces.ProcessResponse, error) {
	...
	if len(req.Attachments) > 0 {
		userMsg.Attachments = req.Attachments
	}

	s.conversation.AddMessage(userMsg)
	...
	s.conversation.AddMessage(assistantMsg)
...
func (s *AgentServiceImpl) LoadHistory(msgs []entities.Message) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.conversation.Context.History = make([]entities.Message, len(msgs))
	copy(s.conversation.Context.History, msgs)
...
func (s *AgentServiceImpl) Reset() {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.conversation.Context.ClearHistory()
	s.conversation.Session.LastActive = time.Now()
	s.conversation.UpdatedAt = time.Now()
}
...
func (s *AgentServiceImpl) UpdateSession(ctx context.Context, updates map[string]any) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.conversation.UpdateSession(ctx, updates)
	return nil
}
...
func (s *AgentServiceImpl) GetConversationStats() ConversationStats {
	...
	stats := ConversationStats{
		TotalMessages: len(s.conversation.Context.History),
		TotalTokens:   s.conversation.GetTotalTokens(),
		Agent:         s.agent.ID,
		CreatedAt:     s.conversation.CreatedAt,
		LastActive:    s.conversation.Session.LastActive,
	}

	// Count tool calls
	for _, msg := range s.conversation.Context.History {
